...
main:
	...
	lb $a1, seed($t0)
generate:
	move $t8, $a0 #store seed in t8
	move $t9, $a1 #store seed pt 2 in t9
	li $v0, 11 #char print seed. a0 should already contain char
	syscall
	#move $a0, $a1
	#li $v0, 11 #char print seed pt 2
	#syscall
	...
	loadbytes:
		...
		addi $t6, $t0, 1 #t6 is 1 more than t0
		lb $t2, strn($t6) # load secound byte
		beq $t2, $0, choosenext #if null in-char, choosenext
		addi $t0, $t0, 1 #iterate for next word
		bne $t1, $t8, loadbytes # if the seed does not match the byte, go to the next
		bne $t2, $t9, loadbytes # AND ^^
	...
	grabnext:
		...
		bne $t1, $t8, grabnext #if not equal to seed do again
		bne $t2, $t9, grabnext # AND^^
